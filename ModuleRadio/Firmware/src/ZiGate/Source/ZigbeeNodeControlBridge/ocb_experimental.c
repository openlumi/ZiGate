...
/* This repo's .zpscfg profiles configure different NIB/AIB table sizes
 * (SecurityMaterialSets, MacTableSize, KeyDescriptorTable) depending on
 * GP_SUPPORT and possibly future build variants. Restore/export never index
 * those tables by a hardcoded size -- every read/write site below already
 * bounds itself against the table's *actual* runtime size (e.g. the loops
 * at "for (i = 0; i < psNib->sTblSize.u8SecMatSet; i++)" and
 * "u8Index < ...->u16SizeOfKeyDescriptorTable"), and the only fixed-index
 * access is security-material slot [0] for the current network key. So
 * bExpLayoutValid() below only needs to confirm there IS at least one
 * security-material slot and that the AIB pointers restore/export actually
 * dereference are populated -- not match a specific table-size fingerprint.
 * That keeps it layout-agnostic across any .zpscfg with enough room, rather
 * than requiring every new build variant to add its sizes to a whitelist. */
#define OCBEXP_MIN_SEC_MATERIALS          (1U)
...
PRIVATE bool_t bExpLayoutValid(ZPS_tsNwkNib *psNib, ZPS_tsAplAib *psAib)
{
    return psNib != NULL && psAib != NULL &&
           ...
           psNib->sTblSize.u8SecMatSet >= OCBEXP_MIN_SEC_MATERIALS &&
